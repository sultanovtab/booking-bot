...
def is_slot_taken(slot_iso: str) -> bool:
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    cur.execute("SELECT 1 FROM bookings WHERE slot_iso=? AND status IN ('pending','confirmed') LIMIT 1", (slot_iso,))
    row = cur.fetchone()
    con.close()
...
def create_booking(*, tg_user_id: int, tg_username: str|None, name: str, phone: str,
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    """, (datetime.utcnow().isoformat(timespec="seconds"), tg_user_id, tg_username, name, phone,
...
def get_booking(booking_id: int):
    ...
    cur = con.cursor()
    cur.execute("""
      SELECT id, tg_user_id, tg_username, name, phone, service_key, service_title, team_size, slot_iso, status
